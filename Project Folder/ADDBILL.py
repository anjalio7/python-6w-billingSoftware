...
class AddBill:
        ...
        def mul(self):
                ...
                self.data=self.selbox1.get()[0]
                self.quan = int(self.ent3.get())
                self.res = database.checkquant(self.data)
                print("Check Result ",self.res)
                ans = self.res[0][0]
                print("Quantity ",self.quan)
                if ans>=self.quan:
                        if self.quan>0:
                                
                                self.remain=ans-self.quan
                                # return True
                                ...
                                if ((self.ent3.get() != '') and (self.ent4.get() != '')):
                                        ...
                                        self.ent5.config(state = 'readonly')
                                        return sum
                                return True
                                # else:
                                #         return 0
                        else:
                                messagebox.showinfo('Alert','Quantity must be greater than 0')
                                return False
                else:
        ...
        def cost(self, e):
                ...
                print(self.selbox1.get())
                self.data1=self.selbox1.get()[0]
                self.res1 = database.checkquant(self.data1)
                self.ent4.insert(0,self.res1[0][1])
                self.ent4.config(state = 'readonly')
        ...
        def firstFrame(self):
                ...
                self.selbox1.bind('<<ComboboxSelected>>',self.cost)
                ...
                self.ent3=Entry(self.mainFrame,textvariable=self.Quantity, validate= "focusout", validatecommand=self.mul,state="readonly")
                self.ent3.place(x=500,y=350,width="150",height="30")

                self.PerPcCost=StringVar()
                self.ent4=Entry(self.mainFrame,textvariable=self.PerPcCost, validate= "focusout", state = "readonly")
                self.ent4.place(x=500,y=300,width="150",height="30")


                self.TotalAmount=StringVar()
                # a = self.mul()
                self.ent5=Entry(self.mainFrame, textvariable= self.TotalAmount, validate= "focusout",bg="white", state = "readonly")
                self.ent5.place(x=500,y=400,width="150",height="30")
        ...
        def abc(self):
                ...
                #         messagebox.showinfo('Alert', 'Please enter Date.')
                        ...
                        if res:
                                ...
                                database.updateProductStock(self.remain,self.res[0][2])
                                self.root.destroy()
        ...
        #         print("Check Result ",self.res)
        #         ans = self.res[0][0]
        #         print("Check Quantity ",ans)
        #         if ans>=self.quan:
        #                 if self.quan>0:
        #                         self.ent4.insert(0,self.res[0][1])
        #                         self.remain=ans-self.quan
        #                         self.ent4.config(state = 'readonly')
        #                         return True
        #                 else:
